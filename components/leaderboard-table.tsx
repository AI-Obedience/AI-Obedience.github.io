...
function TaskTable({ 
  ...
  const meanLabel = taskId === 'pure-color' ? 'color-mean' : taskId === 'image-mask' ? 'shape-mean' : 'mask-mean'
  const meanMax = taskId === 'pure-color' ? 0.3 : 0.6
  const meanLow = taskId === 'pure-color' ? 0.1 : 0.2
  const meanMid = taskId === 'pure-color' ? 0.2 : 0.4
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Badge variant="outline" className="border-primary/50 text-primary">{task.name}</Badge>
    ...
    <section id="leaderboard" className="bg-background py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-12">
          <TaskTable 
            taskId="pure-color" 
            data={pureColorVar1Results} 
            task={mainTasks[0]} 
          />
          
          <TaskTable 
            taskId="image-mask" 
            data={imageMaskResults} 
            task={mainTasks[1]} 
          />
          
          <TaskTable 
            taskId="geometric-shape" 
            data={geometricShapeResults} 
            task={mainTasks[2]} 
          />
        </div>
